...
module Week07.BlackRockPaperScissors
    ( Game (..)
    , GameChoice (..)
    , FirstParams (..)
    , SecondParams (..)
...
data GameChoiceHashes = GameChoiceHashes
    { bsRock :: !ByteString
    , bsPaper :: !ByteString
    , bsScissors :: !ByteString
    }

{-# INLINABLE gameChoiceHash #-}
gameChoiceHash :: GameChoiceHashes -> GameChoice -> ByteString
gameChoiceHash GameChoiceHashes {..} c =
  case c of
    Rock -> bsRock
    Paper -> bsPaper
    Scissors -> bsScissors
...
{-# INLINABLE check #-}
check :: GameChoiceHashes -> GameDatum -> GameRedeemer -> ScriptContext -> Bool
check gcHs (GameDatum bs (Just c)) (Reveal nonce _) _ =
    sha2_256 (nonce `concatenate` gameChoiceHash gcHs c) == bs
check _    _                       _              _ = True

{-# INLINABLE gameStateMachine #-}
gameStateMachine :: Game -> GameChoiceHashes -> StateMachine GameDatum GameRedeemer
gameStateMachine game gcHs = StateMachine
...
mkGameValidator :: Game -> GameChoiceHashes -> GameDatum -> GameRedeemer -> ScriptContext -> Bool
mkGameValidator game gcHs = mkValidator $ gameStateMachine game gcHs

type Gaming = StateMachine GameDatum GameRedeemer

gameChoiceHashes :: GameChoiceHashes
gameChoiceHashes = GameChoiceHashes
    { bsRock = "rock"
    , bsPaper = "paper"
    , bsScissors = "scissors"
...
gameStateMachine' game = gameStateMachine game gameChoiceHashes

gameInst :: Game -> Scripts.ScriptInstance Gaming
gameInst game = Scripts.validator @Gaming
    ($$(PlutusTx.compile [|| mkGameValidator ||])
        `PlutusTx.applyCode` PlutusTx.liftCode game
        `PlutusTx.applyCode` PlutusTx.liftCode gameChoiceHashes)
...
firstGame fp = do
    ...
    let game   = Game
        ...
        c      = fpChoice fp
        bs     = sha2_256 $ fpNonce fp `concatenate` gameChoiceHash gameChoiceHashes c
