...
transition ts s r = case (stateValue s, stateData s, r) of
    (v, p', SetPrice p)  | p >= 0 && isNothing p'       -> Just ( Constraints.mustBeSignedBy (tsSeller ts)
                                                                 , State (Just p) $
                                                                   v <>
                                                                   nft (negate 1)
                                                                 )
    (v, p, AddTokens n)  | n > 0 && isJust p            -> Just ( mempty
                                                                , State p $
                                                                  v              <>
                                                                  nft (negate 1) <>
                                                                  assetClassValue (tsToken ts) n
                                                                )
    (v, p@(Just p'), BuyTokens n)  | n > 0             -> Just ( mempty
                                                                , State p $
                                                                  v                                       <>
                                                                  nft (negate 1)                          <>
    ...
    (v, p, Withdraw n l) | n >= 0 && l >= 0 && isJust p -> Just ( Constraints.mustBeSignedBy (tsSeller ts)
                                                                , State p $
                                                                  v                                       <>
                                                                  nft (negate 1)                          <>
                                                                  assetClassValue (tsToken ts) (negate n) <>
...
startTS mcs token = do
    ...
    let ts = TokenSale
        ...
        client = tsClient ts
    void $ mapErrorSM $ runInitialise client Nothing mempty
    tell $ Last $ Just ts
...
useEndpoints ts = (setPrice' `select` addTokens' `select` buyTokens' `select` withdraw' `select` close') >> useEndpoints ts
  where
    ...
    close'     = handleError logError $ endpoint @"close"      >>= const (close ts)
